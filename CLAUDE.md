...
| `API_KEY` | Custom API key for clients (default: `freebuff-default-key`). `API_KEY` is accepted as a fallback alias. Clients authenticate with `Authorization: Bearer <key>` **or** the `x-api-key` header (`getApiKey()`). |
